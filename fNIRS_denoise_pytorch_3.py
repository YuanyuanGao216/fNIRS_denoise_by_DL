...
class Net_8layers(nn.Module):
    def __init__(self):
        ...
        self.conv4 = BasicConv1d(64, 128, 3, dilation=2, padding = 2)
        self.conv5 = BasicConv1d(128, 128, 3, dilation=2, padding = 2)
    ...
    def forward_half(self, x):
        ...
        x = self.pool(x)
        b, c, l = x.size()
#         x = F.relu(self.fc_1(x.view(b, c*l)))
#         x = self.dout(x)
#         x = F.relu(self.fc_2(x))
        x = self.up(x.view(b, c, l))
        x = self.conv6(x)
        x = self.conv7(x)
        x = self.up(x)
        x = self.conv8(x)
