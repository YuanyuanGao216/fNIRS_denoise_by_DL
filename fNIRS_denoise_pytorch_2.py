...
class Net_8layers(nn.Module):
    ...
    def forward(self, x):
        ...
        # x1 = F.relu(self.conv2(x1)) #extra
        # x2 = F.relu(self.conv2(x2)) #extra
        # print(x1.shape)
        ...
        # x1 = F.relu(self.conv3(x1))#extra
        # x2 = F.relu(self.conv3(x2))#extra
        
        x1 = self.pool(F.relu(self.conv3(x1)))
        x2 = self.pool(F.relu(self.conv3(x2)))
        # print(x1.shape)
        
        # x1 = F.relu(self.conv4(x1))#extra
        # x2 = F.relu(self.conv4(x2))#extra
        
        x1 = self.pool(F.relu(self.conv4(x1)))
        x2 = self.pool(F.relu(self.conv4(x2)))
        # print(x1.shape)
        
        x1 = F.relu(self.conv5(x1))#extra
        x2 = F.relu(self.conv5(x2))#extra
        
        x1 = self.up(F.relu(self.conv5(x1)))
        x2 = self.up(F.relu(self.conv5(x2)))
        # print(x1.shape)
        
        # x1 = F.relu(self.conv6(x1))#extra
        # x2 = F.relu(self.conv6(x2))#extra
        
        x1 = self.up(F.relu(self.conv6(x1)))
        x2 = self.up(F.relu(self.conv6(x2)))
        # print(x1.shape)
        
        # x1 = F.relu(self.conv7(x1))#extra
        # x2 = F.relu(self.conv7(x2))#extra
        
        x1 = self.up(F.relu(self.conv7(x1)))
        x2 = self.up(F.relu(self.conv7(x2)))
        # print(x1.shape)
        
        # x1 = F.relu(self.conv8(x1))#extra
        # x2 = F.relu(self.conv8(x2))#extra
...
if __name__ == '__main__':
    ...
    for file_leave_out, subdir in enumerate(subdirs[1:]):
        ...
        # %%
        Hb_act = dc_act[0, file_leave_out]
        n = Hb.shape[0] // 512
        ...
        lowest_val_loss = 1e6;
        hdf5_filepath = os.path.join(subdir, "8layers")
    ...
    #                 loss = loss1 + 0.01 *(loss2 + loss3)
        ...
        plt.title('loss1')
        figurepath = os.path.join(subdir, model_name+"_1"+".png")
        ...
        plt.legend([tl,vl],['training loss', 'validation loss'],)
        plt.title('loss2')
        figurepath = os.path.join(subdir, model_name+"_2"+".png")
        ...
        plt.legend([tl,vl],['training loss', 'validation loss'],)
        plt.title('loss3')
        figurepath = os.path.join(subdir, model_name+"_3"+".png")
        ...
        Y_test = Y_test/1000000
        savefilepath = os.path.join(subdir, "Test_NN_" + model_name+".mat")
        ...
        savefilepath = os.path.join(subdir, "Real_NN_" + model_name+".mat")
        ...
        savefilepath = os.path.join(subdir, "Real_NN_" + model_name+"_act.mat")
        ...
        X, = plt.plot(X_real_example,'b')
        Y, = plt.plot(Y_real_example,'r')
        figurepath = os.path.join(subdir, "Y_real_example_act.png")
        # figurepath = "Figures/Y_real_example_act.png"
        plt.savefig(figurepath, transparent=True)
        print('Y_real_act shape is ',Y_real_act.shape)
        
        # if file_leave_out == 0:
        #     RealData_newdata = scipy.io.loadmat('Processed_data/RealData_newdata.mat')
        #     net_input_newdata = RealData_newdata['net_input']
        #     dc_newdata = net_input_newdata['dc']
        #     X_real_newdata = []
        #     for i in range(np.array(dc_newdata).shape[1]):
                
                
        #         Hb_newdata = dc_newdata[0, i]
                
        #         n = Hb_newdata.shape[0] // 512
                
        #         HbO_newdata = np.transpose(np.squeeze(Hb_newdata[:n*512, 0, :]))
        #         HbR_newdata = np.transpose(np.squeeze(Hb_newdata[:n*512, 1, :]))
                
        #         HbO_newdata = np.reshape(HbO_newdata,(-1,512))
        #         HbR_newdata = np.reshape(HbR_newdata,(-1,512))
                
        #         X = np.concatenate((HbO_newdata,HbR_newdata),axis = 1)
        #         X_real_newdata.append(X)
                
        #     X_real_newdata = np.concatenate(X_real_newdata, axis=0)
        #     X_real_newdata = X_real_newdata*1000000
        #     X_real_newdata = X_real_newdata[:,:]
        #     real_set_newdata = Dataset(X_real_newdata)
        #     realloader_newdata = torch.utils.data.DataLoader(
        #         dataset = real_set_newdata, 
        #         batch_size=32,
        #         sampler = tordata.SequentialSampler(real_set_newdata),
        #         num_workers=2)
        #     Y_real_newdata = []
            
        #     for i, data in enumerate(realloader_newdata, 0):
        #         inputs = data[0]
        #         outputs = net(inputs.cuda().float())
        #         Y_real_newdata.append(outputs.cpu().data.numpy())
        #     Y_real_newdata = np.concatenate(Y_real_newdata, axis=0)
        #     Y_real_newdata = Y_real_newdata/1000000
        #     savefilepath = os.path.join('Processed_data', "Real_NN_" + model_name+"_newdata.mat")
        #     scipy.io.savemat(savefilepath,{'Y_real_newdata': Y_real_newdata})
