...
figure_flag = 0;
% for file = 2:length(Files) - 1 % save the last one for testing
for file = 2:2 % save the last one for testing
...
%         for Ch = 1:n_Ch
        for Ch = 1:1
            ...
            period = 512;
            for start_point = 1:1000:(length(tInc) - period)
...
%                     end
                    ...
                    % plot an example
                    if file == 2 && Ch == 1
                          plot_real_data(t,dc,tInc,Ch)
%                         plot_real_data(t,dc,tInc,channel,t_span,Resting_HbO,sim_data_HbO)
%                         figure_flag = 1;
%                         loc_example = size(simulated_HbO,1) + 1;
%                         save('Processed_data/plot_Resting.mat','t','dc','tInc','channel','t_span','Resting_HbO','sim_data_HbO','loc_example');
                    end
